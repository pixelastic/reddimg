...
describe('module', () => {
  describe('getPictureFromPost', () => {
    it('should return the source if not upperBound defined', async () => {
      const input = {
        data: {
          preview: {
            images: [
    ...
    it('should return the resolution width the width closest but above the upperBound', async () => {
      ...
      const actual = module.getPictureFromPost(input, minWidth);
  ...
  describe('mergeData', () => {
    it('should add records to empty data', async () => {
      ...
      const actual = module.mergeData(local, remote);
