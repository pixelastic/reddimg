...
module.exports = {
  ...
  getRecordFromPost(rawPost) {
    const picture = this.getPictureFromPost(rawPost, 600);
    if (!picture) {
      return false;
    }
   ...
   **/
  getPictureFromPost(rawPost, minWidth) {
    const firstImg = _.get(rawPost, 'data.preview.images[0]', false);
    if (!firstImg) {
      return false;
    }
    const sourceUrl = _.get(firstImg, 'source.url');

    // If a minWidth if specified, we take the closest url
    let previewUrl;
    if (minWidth) {
      previewUrl = _.chain(firstImg)
        .get('resolutions', [])
        .sortBy('width')
        .find(resolution => {
    ...
    const finalUrl = previewUrl || sourceUrl;
    return _.replace(finalUrl, /&amp;/g, '&');
  },
  /**
  ...
  /**
   * Returns the list of subreddit configured on _data
   * @returns {Array} List of subreddits
   **/
  async getLocalSubreddits() {
    const dataFiles = await glob('src/_data/*.json');
